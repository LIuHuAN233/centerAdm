        ...
        <div class="col-md-9 col-md-offset-2" style="background-color: #ffffff;padding: 20px 40px;">
            <div class="page-header">
                ...
                <div style="color:#8B8B7A;height: 30px;width: 100%;line-height: 30px" >
                    ...
                    <span class="text-info" style="margin-left: 30px;" id="createName"></span>
                    <span style="margin-left: 500px" id="blogTags">文章标签:
                    </span>
                </div>

            </div>
        </div>
        <div class="col-md-9 col-md-offset-2" id="content">
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        /*显示文章*/
        $.ajax({
            url:"/attach/showOneBlog",
            data:{"type":"${type}","id":${id}},
            success:function (result) {
                var time = new Date(result.createDate);
                var day = ("0" + time.getDate()).slice(-2);
                var month = ("0" + (time.getMonth() + 1)).slice(-2);
                var bdate = time.getFullYear() + "-" + (month) + "-" + (day);
                $("#createDate").append(bdate);
                $("#createName").append(result.createName);
                $("#blogTitle").append(result.title);
                var tagArr = (result.tags).split(",");
                for (var i=0;i<tagArr.length;i++){
                    $("#blogTags").append("<a href=\"#\" style='margin-left: 10px;'><button class=\"btn-primary btn\">"+tagArr[i]+"</button></a>");
                }
