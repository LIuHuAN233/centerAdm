            ...
            <form class="form-horizontal" role="form">
                <fieldset>
                    ...
                    <div class="form-group" style="margin-bottom: 30px;margin-top: 30px;">
                        <label class="col-sm-2 control-label" for="ds_host" style="padding-left: 10px;">子项目名</label>
                        <div class="col-sm-10">
...
```</textarea>
                ...
                <fieldset id="tasks" style="margin-bottom: 30px;">
                    <legend>添加任务</legend>
        ...
        $froms.each(function () {
            ...
            if (taskName=="" || taskCode=="" || taskBegin=="" || taskEnd==""){
                ...
                if (taskBegin<$("#beginTime").val()){
                    ...
                    rqs[12]=rqs[12]+taskName+",";
                    rqs[13]=rqs[13]+taskCode+",";
                    rqs[14]=rqs[14]+taskBegin+",";
                    rqs[15]=rqs[15]+taskEnd+",";
                }
            }
        });
        /*for (var i=1;i<taskId+1;i++){
            if ($("#taskName"+i).val()=="" || $("#beginTime"+i).val()=="" || $("#stopTime"+i).val()=="" || $("#taskCode"+i).val()==""){
                check++;
            }else {
                if ($("#beginTime"+i).val()<$("#beginTime").val()){
                    check++
                }else if ($("#stopTime"+1).val()>$("#stopTime").val()){
                    check++;
                }else if($("#beginTime"+i).val()>$("#stopTime"+1).val()){
                    check++;
                }else {
                    rqs[10]=rqs[10]+$("#taskName"+i).val()+",";
                    rqs[12]=rqs[12]+$("#beginTime"+i).val()+",";
                    rqs[13]=rqs[13]+$("#stopTime"+1).val()+",";
                    rqs[11]=rqs[11]+$("#taskCode"+i).val()+",";
                }
            }
        }*/


        var startDate = new Date(rqs[4]);
        var endDate = new Date(rqs[5]);
        if ($("#startTimeF").val()>(startDate.getTime()-60*60*8*1000)){
            alert("开始时间不能小于父项目开始时间！")
        }else if($("#stopTimeF").val()<(endDate.getTime()-60*60*8*1000)){
            alert("结束时间不能晚于父项目结束时间！")
