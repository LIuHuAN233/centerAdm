...
<div class="down-main">
    ...
    <div class="row">
        ...
        <div class="col-md-9 col-md-offset-2" style="margin-top: 15px;">
            <h1 style="margin-left: 21px;margin-bottom: 15px;" class="col-sm-3">进行中的项目</h1>
    ...
    window.operateEvents = {
        ...
        'click .addRate': function (e, value, row, index) {
            if (row.pStatus==0){
                ...
                alert("当前项目已被提交，不可新增进度，可修改已提交进度")
            }

        }
        ,
        'click .updateRate': function (e, value, row, index) {
            window.location.href="/gotoShowOneProject/"+row.id;
        },
        'click .finish': function (e, value, row, index) {
            ...
            for (var i=0;i<leaders.length;i++){
                if (userName==leaders[i]){
                    if (confirm("确定完成此项目吗？将同时完成所有子项目。")) {
                        $.ajax({
                            url: "/finishOneProject",
                            data: {"id": row.id},
                            success: function (result) {
                                if (result=="success"){
        ...
        'click .uploadFile': function (e, value, row, index) {
            window.location.href="/gotoLeaderUpdateFile/"+row.id;
        }

    }
    $("#selectByD").click(function () {
        ...
        columns: [
            ...
            /*{
                ...
                formatter:function (value,row,index) {
                    if (row.pLevel ==2){
                        return ['&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="/gotoShowOneProject/'+row.id+'">'+row.pName+'</a>'].join("");
                    }
                    if (row.pLevel ==3){
                        return ['&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="/gotoShowOneProject/'+row.id+'">'+row.pName+'</a>'].join("");
                    }
            ...
            if (row.pFatherName==""){
                if(row.pIsFinish==0){
                    return [
                        ...
                        '</a>',
                        '<a class="addRate ml10" style="color:black;margin-left: 15px" href="javascript:void(0)" title="添加进度">',
                        '<i class="glyphicon glyphicon-menu-hamburger" style="color: #398439"></i>',
                        '</a>',
                        ...
                        '</a>',
                        '<a class="uploadFile ml10" style="color:black;margin-left: 15px" href="javascript:void(0)" title="上传附件">',
                        '<i class="glyphicon glyphicon-folder-open" style="color:#B8860B"></i>',
                        '</a>'
                    ].join('');
                }/*else {
                return [
                ...
                if (row.pStatus==0){
                    ...
                    return [
                        ...
                        '<span class="glyphicon glyphicon-saved" style="color:#d58512"></span>',
                        '</a>',
                        '<a class="updateRate ml10" style="color:black;margin-left: 15px" href="javascript:void(0)" title="修改进度">',
                        '<i class="glyphicon glyphicon-edit" style="color: #2aabd2"></i>',
                        '</a>',
                        '<a class="showGantt ml10" style="color:black;margin-left: 15px" href="javascript:void(0)" title="展示甘特图">',
                        '<i class="glyphicon glyphicon-picture" style="color:#2F4F4F"></i>',
                        '</a>',
            ...
            if (row.pFatherName==""){
                if(row.pIsFinish==0){
                    return [
                        ...
                        '<i class="glyphicon glyphicon-picture" style="color:#2F4F4F"></i>',
                        '</a>',
                        '<a class=" ml10" style="color:black;margin-left: 15px" href="javascript:void(0)">',
                        '<i class="glyphicon glyphicon-picture" style="color:transparent;"></i>',
                        '</a>'
                    ].join('');
                }/*else {
                return [
